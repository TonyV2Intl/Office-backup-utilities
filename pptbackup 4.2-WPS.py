...
def save_open_ppt_files(save_folder):   #定义保存函数，参数save_folder是备份文件的存储路径
    ...
    ppt_app=win32.GetObject(Class='KWPP.Application')   #捕获当前打开的WPS演示实例
    ...
    for ppt in presentations:   #遍历集合
        ...
        if os.path.exists(new_ppt_path):   #检查备份文件是否已存在
            if SaveAs_method_activated[ppt_name] == True:   #如果SaveAs方法已被激活，则不再使用复制方法
                runid+=1
                print(time.strftime('[%H:%M:%S-#') + str(runid) + '] ' + ppt_name + ' has already existed in ' + save_folder + ', skipped backup (SaveAs method activated)')   #打印带时间戳和运行次数的跳过信息
                continue   #跳过此次备份
            if file_skip_count[ppt_name] < 5:  # 仅当同一文件连续跳过5次时才允许重新备份
                file_skip_count[ppt_name] += 1   #该文件的跳过计数器累加
                runid+=1   #运行计数器累加
                if file_skip_count[ppt_name] == 5:   # 如果跳过次数达到5次，打印提示信息
                    print(time.strftime('[%H:%M:%S-#') + str(runid) + '] ' + ppt_name + ' has already existed in ' + save_folder + ', skipped backup (skipped times: ' + str(file_skip_count[ppt_name]) + ', this file will be backed up again during the next request)')   #打印带时间戳和运行次数的跳过信息
                else:
                    print(time.strftime('[%H:%M:%S-#') + str(runid) + '] ' + ppt_name + ' has already existed in ' + save_folder + ', skipped backup (skipped times: ' + str(file_skip_count[ppt_name]) + ')')   #打印带时间戳和运行次数的跳过信息
                continue   #跳过此次备份

        runid+=1   #运行计数器累加
        print(time.strftime('[%H:%M:%S-#') + str(runid) + '] Start to backup ' + ppt_name + ' to ' + save_folder)   #打印备份开始信息
    ...
    if not any_backup_performed and len(presentations) == 0:   #检查变量值，如果没有可备份PPT，打印此条信息
        runid+=1   #运行计数器累加
        print(time.strftime('[%H:%M:%S-#') + str(runid) + '] No ppt available now (Normal request)')   #打印带时间戳和运行次数的运行信息
...
while True:   #无限循环
    try:
        ...
        if type(e).__name__=='com_error':   #捕获无打开的PowerPoint实例而产生的的异常
            runid+=1   #运行计数器累加
            print(time.strftime('[%H:%M:%S-#') + str(runid) + '] No ppt available now (WPS application not detected)')   #打印带时间戳和运行次数的异常信息
            time.sleep(sleeptime)   #等待下次轮询
        else:   #打印出其他错误并继续轮询
            runid+=1   #运行计数器累加
