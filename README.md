...
flowchart TD
  ...
  n_3("初始化日志系统")
  n_4("设置控制台窗口显示/隐藏")
  n_5("初始化全局变量（文件跳过计数 / SaveAs激活标记 / 会话出现标记）")
...
flowchart TD
    ...
    n_22 --> n_23
    n_23 -->|成功| n_24
    n_23 -->|失败 FileNotFoundError| n_25
    n_24 --> n_26
    n_25 --> n_26
    n_26 -->|是| n_28
    n_26 -->|否| n_32
...
```mermaid
flowchart TD
    subgraph n_init [OpenList 初始化流程]
        n_i1("文件添加到上传队列")
        n_i2{"OpenList已初始化
        或正在初始化?"}
        n_i3("登录OpenList")
    ...
    subgraph n_upload [OpenList 上传流程]
        n_u1("上传线程启动")
        n_u2{"OpenList是否已初始化?"}
        n_u3("等待初始化完成
        退出线程")
...
| 技术模块 | 实现方式与细节 |
| --------- | ------------- |
| **COM 组件交互** | 通过 `pywin32` 库的 `win32com.client` 与 Office 应用交互；PPT/Word 使用 `Dispatch` 方式，WPS 使用 `GetObject` 方式；支持通过 `SaveAs` 方法处理移动存储介质移除的异常场景 |
| **文件操作** | 使用 `shutil.copy2` 复制文件并保留元数据；`os.utime` 自定义修改时间戳；自动处理只读文件属性（`remove_readonly`）；`shutil.copytree` 实现文件夹精确备份，支持 `dirs_exist_ok` |
...
| **异常处理** | 分层异常捕获：COM 异常、文件操作异常、网络异常；全局异常处理器记录未捕获错误；关键操作失败时优雅降级（如 OpenList 连接失败自动禁用上传功能） |
...
> **精确备份（文件夹备份）说明**
>
> 精确备份功能用于将整个文件夹（包括其子目录和所有文件）完整复制到指定的目标路径。与常规的 Office 文档备份不同，此功能不依赖于 COM 接口，适用于任意类型的文件备份。
>
> **配置方法：**
>
> 1. 在配置文件中将 `accurate_backup_enable` 设为 `true`，或通过 ConfigEditor 勾选"启用精确备份"
> 2. 设置 `accurate_backup_source_path`（源路径）为要备份的文件夹绝对路径
> 3. 设置 `accurate_backup_target_path`（目标路径）为备份存放的文件夹绝对路径，在Config Editor中可以根据源路径在后方自动追加`源文件夹名称-backup`
> 4. 确保源路径和目标路径均不为空，否则精确备份功能将自动禁用并记录日志；备份时，如果源文件夹不存在则跳过本次备份等待下次轮询
>
> **运行机制：**
>
> · 精确备份在每次轮询周期的独立守护线程中运行，不会阻塞主循环  
> · 使用 `shutil.copytree` 实现完整目录复制，支持 `dirs_exist_ok=True`，允许备份已经有文件的目录
...
```json
{
    ...
    "accurate_backup_target_path": "",   //目标路径，备份存放的文件夹绝对路径
    //托盘图标、控制台行为与日志保存设置
