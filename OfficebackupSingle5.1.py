...
default_config = {
    ...
    "upload_to_openlist_enable": True,   #上传到openlist云盘功能
    #openlist云盘参数
...
if config.get('upload_to_openlist_enable'):   #如果上传功能开启，则读取相关参数
    openlist_url = config.get('openlist_url')  # openlist服务器地址
    openlist_username = config.get('openlist_username')  # openlist用户名
    openlist_password = config.get('openlist_password')  # openlist密码
    openlist_target_folder = config.get('openlist_target_folder')  # openlist目标文件夹路径
'''behavior = config.get('tray_left_click_behavior')  # 托盘图标左键点击行为（默认为打开控制台）（无法生效）'''



try:   #尝试导入openlist库
    from openlist import Client   #导入openlist库的Client类，用于与openlist云盘交互
except ImportError:
    log_print("Openlistapi lib not found, force disabled upload function")
    config['upload_to_openlist_enable'] = False   #强制禁用上传功能

if not openlist_url or not openlist_username or not openlist_password:   #检查openlist_url、用户名和密码是否为空，若为空则强制禁用上传功能
...
def upload_to_openlist():   #定义上传函数
    import asyncio  # 导入asyncio库，用于处理异步操作
    global upload_queue  #声明全局上传队列变量
    if not config.get('upload_to_openlist_enable'):   #如果上传功能未启用，则跳过本次上传
        return
    ...
    async def async_upload():
        ...
        while upload_queue:
            ...
            for (upload_file, upload_source_path) in list(upload_queue):
                ...
                # 构建目标文件路径，确保使用正斜杠
                target_file_path = os.path.join(openlist_target_folder, upload_file)
                # 将反斜杠替换为正斜杠，确保路径格式正确
                ...
                try:
                    ...
                    async with Client(openlist_url) as client:
                        ...
                        # 检查文件是否已存在，存在则删除
                        try:
                            # 使用info方法检查文件是否存在，如果不存在会抛出异常
                            await fs.info(target_file_path)
                            # 如果没有抛出异常，说明文件存在，进行删除
                            await fs.remove(target_file_path)
                        ...
                        # 上传文件
                        await fs.upload_file(upload_source_path, target_file_path)
...
while True:   #主线程无限循环，防止程序退出
    ...
    if config.get('upload_to_openlist_enable'):   #检查上传到openlist云盘功能是否启用
