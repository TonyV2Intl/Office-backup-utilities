...
default_config = {
    ...
    #指定间隔时间，单位为秒
    "backup_interval": 60,   #每分钟轮询一次（默认）
    "max_skipping_time": 15,   #每个文件允许连续跳过15次备份，即15分钟备份一次已有文件（默认）
    "upload_interval": 300,   #每5分钟上传一次云盘（默认）
    "accurate_backup_interval": 60,   #每分钟尝试进行一次文件夹精确备份（默认）
    #功能开启或禁用
...
try:   #读取配置文件
    with open('OfficebackupMultiConfig.json', 'r', encoding='utf-8') as f:   #尝试读取配置文件（只读）
        config = json.load(f)
    ...
    with open('OfficebackupMultiConfig.json', 'w', encoding='utf-8') as f:   #在当前目录下根据默认配置文件创建（写入）
...
#从配置文件读取变量
backup_interval=config.get('backup_interval')   #轮询间隔（默认为60秒）
...
if config.get('upload_to_123pan_enable'):   #如果上传功能开启，则读取相关参数
    upload_interval=config.get('upload_interval')   #上传间隔（默认为300秒）
...
def request_access_token():   #定义获取access_token函数
    ...
    try:   #尝试获取123云盘的access_token
        ...
        with open('OfficebackupMultiConfig.json', 'w', encoding='utf-8') as f:   #将access_token写入配置文件
...
if config.get('accurate_backup_enable'):  # 检查精确备份功能是否启用
    ...
    accurate_backup_interval = config.get('accurate_backup_interval')  # 获取精确备份间隔
    if not source_path and target_path:   #如果精确备份功能开启但源路径为空或目标路径为空，则强制禁用精确备份功能
        ...
        with open('OfficebackupMultiConfig.json', 'w', encoding='utf-8') as f:   #将禁用精确备份功能写入配置文件
...
def save_open_ppt_files(ppt_save_folder):   #定义ppt保存函数，参数ppt_save_folder是备份文件的存储路径
    ...
    while True:   #无限循环
        try:
            ...
            time.sleep(backup_interval)   #等待下次轮询
...
def upload_to_123pan():   #定义上传函数
    ...
    while True:   #无限循环
        ...
        time.sleep(upload_interval)   #等待下次检查上传队列是否有新文件
...
def accurate_backup():   #定义精确备份函数
    while True:   #无限循环
        ...
        time.sleep(accurate_backup_interval)  #等待下次精确备份
...
ppt_saving_task = threading.Thread(target=save_open_ppt_files, args=(ppt_save_folder,))   #创建线程对象，目标函数为save_open_ppt_files，传入备份路径参数
word_saving_task = threading.Thread(target=save_open_word_files, args=(word_save_folder,))   #创建线程对象，目标函数为save_open_word_files，传入备份路径参数
WPS_saving_task = threading.Thread(target=save_open_WPS_files, args=(ppt_save_folder,))   #创建线程对象，目标函数为save_open_WPS_files，传入备份路径参数
upload_task = threading.Thread(target=upload_to_123pan)   #创建线程对象，目标函数为upload_to_123pan
icon_task = threading.Thread(target=run_icon)   #创建线程对象，目标函数为icon.run
icon_task.daemon = True   #设置为守护线程（随主程序终止而自动结束）
accurate_backup_task = threading.Thread(target=accurate_backup)  # 创建线程对象，目标函数为accurate_backup

if config.get('ppt_backup_enable'):   #检查PPT备份功能是否启用
    ppt_saving_task.start()   #启动线程
    time.sleep(1)   #等待1秒，确保PPT备份线程启动
if config.get('word_backup_enable'):   #检查Word备份功能是否启用
    word_saving_task.start()   #启动线程
    time.sleep(1)   #等待1秒，确保Word备份线程启动
if config.get('wps_backup_enable'):   #检查WPS备份功能是否启用
    WPS_saving_task.start()   #启动线程
    time.sleep(1)   #等待1秒，确保WPS备份线程启动
if config.get('upload_to_123pan_enable'):   #检查上传到123云盘功能是否启用
    upload_task.start()   #启动线程
    time.sleep(1)   #等待1秒，确保上传线程启动
if config.get('accurate_backup_enable'):  # 检查精确备份功能是否启用
    accurate_backup_task.start()  # 启动线程
icon_task.start()   #启动托盘图标线程
icon_task.join()   #使用托盘图标线程维持主线程运行
